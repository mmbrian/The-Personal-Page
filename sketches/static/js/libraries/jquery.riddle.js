...
(function($){
    ...
    hideAllCharacters = function () {
        if (riddle_options.fastHide) {
            for (var i=0; i<paragraph_details[0]; i++) {
                $('.p' + i).children().removeClass('rvisible').addClass('rhidden');
            }
        } else {
                    ...
                    setTimeout(
                        function() {
                            $('.p' + _p + 'c' + (_mid + r)).transition(
                                ...
                                function() {
                                    $(this).removeAttr('style').removeClass('rvisible').addClass('rhidden');
                                    totalVisibleChars--;
                                }
                            );
                        },
    ...
    displaySingleCharacter = function (charInd, isSymbol) {
        $('.p' + curr_paragraph + 'c' + charInd).transition(
            ...
            function() {
                $(this).removeAttr('style').removeClass('rhidden').addClass('rvisible');
                if (!symbol_detected){
    ...
    startAnimation = function () {
        riddle_options.extras.forEach(function(id) {
            $('#' + id).removeClass('rvisible').addClass('rhidden');
        });
        hideAllCharacters();
    ...
    $.fn.riddle = function(options) {
        ...
        $("<style type='text/css'> .rvisible{opacity: 1;} .rhidden{opacity: 0;} </style>").appendTo("head");
